...
def eval_model(args):
                ...
                try:
                    ...
                    for entry in existing:
                        if "imagename" in entry:
                            already_done.add(entry["imagename"])
                except Exception:
                    pass
        except Exception:
            pass
    ...
    for i, image_file in enumerate(image_files):
        if image_file in already_done:
            print(f"Skipping already processed image: {image_file}")
            continue

        image_file_path = os.path.join(test_images_path, image_file)
        print(f"Processing image {i+1}/{len(image_files)}: {image_file_path}")
        try:
