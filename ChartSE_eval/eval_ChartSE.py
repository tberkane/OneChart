...
# pred_type = "csv"
pred_type = "json"
with open("data/example_pred(use_this).json", "r") as questions_file:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--infer_result_dir", required=False, help="Path to the inference result data"
    )
    ...
    for item in tqdm(questions_data):
        ...
        if imgname in title_to_values.keys():
            if pred_type == "json":
                try:
                    complete_json_answer = json.loads(
                        complete_json_string(item["answer"])
                    )
    ...
    (
        ...
        ap_75_high,
        ap_90_high,
    ) = csv_eval(csv_pred_set, csv_gt_set, easy, pred_type=pred_type)
    print("\n")
